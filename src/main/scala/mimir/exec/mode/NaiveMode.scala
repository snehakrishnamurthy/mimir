...
class NaiveMode (seeds: Seq[Long] = (0l until 10l).toSeq)
  ...
  def ReplaceMissingLensToSampler(query: Operator, models: (String => Model), shift: Int): (Operator, Set[String]) =
  {
    ...
    query match {
      ...
      case Join(lhsOldChild, rhsOldChild) => {
        val (lhsNewChild, lhsNonDeterministicInput) = ReplaceMissingLensToSampler(lhsOldChild, models,shift)
        val (rhsNewChild, rhsNonDeterministicInput) = ReplaceMissingLensToSampler(rhsOldChild, models,shift)
      ...
      case ( LeftOuterJoin(_,_,_) | Annotate(_, _) | ProvenanceOf(_) | Recover(_, _) ) =>
        throw new RAException("Naive mode presently doesn't support LeftOuterJoin")
    }
  }
